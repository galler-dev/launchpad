...
contract("Launchpad", (accounts) => {
    it("add campaign", async () => {
        ...
        truffleAssert.passes(launchpad.addCampaign(nft.address, accounts[1], web3.utils.toWei("0.001", "ether"), 10, 1644390809, 1744390809, 1, 2));
        truffleAssert.reverts(launchpad.addCampaign(nft.address, accounts[1], web3.utils.toWei("0.001", "ether"), 10, 1644390809, 1744390809, 1, 2), "contract address already exist");
        truffleAssert.passes(launchpad.updateCampaign(nft.address, accounts[1], web3.utils.toWei("0.001", "ether"), 10, 1644390809, 1744390809, 2, 2));
    });
    ...
    it("mint", async () => {
        ...
        await truffleAssert.passes(launchpad.addCampaign(nft.address, accounts[1], web3.utils.toWei("0.001", "ether"), 10, 1644390809, 1744390809, 1, 2));
    ...
    it("mint per address", async () => {
        ...
        // addCampaign(address contractAddress_, uint256 price_, uint256 totalSupply_, uint256 listingTime_, uint256 expirationTime_, uint256 maxBatch_)
        await truffleAssert.reverts(launchpad.mint(nft.address, 1), "contract not register");
        await truffleAssert.passes(launchpad.addCampaign(nft.address, accounts[1], web3.utils.toWei("0.001", "ether"), 10, 1644390809, 1744390809, 3, 2));
    ...
    it("supply limit", async () => {
        ...
        let nft = await LaunchpadNFT.new("LaunchpadNFT", "LNFT", "https://metadata.com/nft/", launchpad.address, 10);
        // addActivity(address contractAddress_, uint256 price_, uint256 totalSupply_, uint256 listingTime_, uint256 expirationTime_, uint256 maxBatch_)
        await truffleAssert.passes(launchpad.addCampaign(nft.address, accounts[1], web3.utils.toWei("0.001", "ether"), 10, 1644390809, 1744390809, 10, 10));
    ...
    it("time limit", async () => {
        ...
        let nft = await LaunchpadNFT.new("LaunchpadNFT", "LNFT", "https://metadata.com/nft/", launchpad.address, 10);
        // addActivity(address contractAddress_, uint256 price_, uint256 totalSupply_, uint256 listingTime_, uint256 expirationTime_, uint256 maxBatch_)
        await truffleAssert.passes(launchpad.addCampaign(nft.address, accounts[1], web3.utils.toWei("0.001", "ether"), 10, 1744390809, 1744390809, 10, 10));
        await truffleAssert.reverts(launchpad.mint(nft.address, 10, {value : web3.utils.toWei("0.01", "ether")}), "activity not start");
        await truffleAssert.passes(launchpad.updateCampaign(nft.address, accounts[1], web3.utils.toWei("0.001", "ether"), 10, 1644390809, 1644390809, 10, 10));
